...
function DoctorInfo() {
  ...
  const check = (slot) => {
    const t = userInfo.appointments.filter(
      (app) =>
        app.slot === slot.slot &&
        apponitmentDate === new Date(app.date).toLocaleDateString("en-US")
    );
          ...
          <Row>
            ...
            <Col md={{ span: 6, offset: 3 }}>
              <Card className="p-2">
                ...
                {dup.length !== 0 ? (
                  `You Already Have Appointment with ${doctor.specialization.name} on ${apponitmentDate}`
                ) : (
                  <>
                            ...
                            <div className="room" key={slot.slot}>
                              ...
                              <p>
                                ...
                                : "(Not Available)"} */}
                                  {check(slot) ? (
                                    <p>
                                      "You already have another appointment during
                                      this time"
                                    </p>
                                  ) : isAvailable(slot) ? (
                                    "(Available)"
